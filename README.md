# Температурные нормы реакции ключевых метаболических ферментов у _Eulimnogammarus verrucosus_ (Jakob, 2021)
## Работа выполнена в рамках дисциплины "Воспроизводимые исследования в биологии":
## GRAPHICAL ABSTRACT
![](Graph_Abstr.png)
## РЕЗЮМЕ
_Eulimnogammarus verrucosus_ (эулимногаммарус бородавчатый) – эндемичный вид амфипод озера Байкал (рис. 1). Впервые вид был описан Герстфельдом в 1858 году как _Gammarus verrucosus_, в 1874 году Б. И. Дыбовским перенесён в род _Eulimnogammarus_. Обитает на глубине от 0 до 25 м.
...
Данное исследование направлено на выявление молекулярно-биохимических основ этой уязвимости путём анализа активности ключевых метаболических ферментов (цитратсинтазы, цитохром-с-оксидазы, лактатдегидрогеназы и пируваткиназы) и дифференциальной экспрессии генов в условиях постепенного потепления (0.8 °C/день), имитирующего сценарий климатических изменений.

Настоящее исследование демонстрирует, что эндемичная байкальская амфипода _Eulimnogammarus verrucosus_ обладает узкой температурной толерантностью, что делает её уязвимой к климатическому потеплению. Анализ показал, что температурный оптимум для ключевых ферментов аэробного метаболизма (цитохром-с-оксидазы, цитратсинтазы и пируваткиназы) находится на уровне 15,6 °C, что совпадает с экологическим температурным оптимумом вида. При этом активность лактатдегидрогеназы продолжала расти вплоть до 23,6 °C, указывая на особую роль анаэробного гликолиза при тепловом стрессе.

![](E_ver.png)

Рис. 1. Внешний вид объекта исследования. Источник фотографии: [https://festivalnauki.ru/media/articles/interesno-o-nauke/neizvedannyy-baykal/]
...
`NCBI Blastn`
## В ходе работы были скачаны и использованы следующие образцы из базы данных SRA:
`SRR8205839`
`SRR8205845`
`SRR8205840`
`SRR8205846`
...
### Скачивание данных
_Указываем путь к исполняемому файлу программы sratoolkit:_
...
#### На примере активности фенолоксидазы (PO) у _E. coli_ и _E. verrucosus_ (рис. 2)
```
...
```
 ...
 ![](PO_with_stats.png)
Рис. 2. Активность фенолоксидазы (PO) у _E. coli_ и _E. verrucosus_.
#### Дополнительные скрипты (заливка и отображение значений) (рис. 3)
```
...
![](Supplementary.png)
Рис. 3. Активность фенолоксидазы (PO) у _E. coli_ и _E. verrucosus_ (в цвете).
#### Построение графиков активности ключевых метаболических ферментов при различных температурах: лактатдегидрогеназы (LDH), цитратсинтазы (CS), цитохром-c-оксидаза (COX) и пируваткиназы (PK) у _E. verrucosus_ (рис. 4) (Jakob et al, 2021)
```
setwd("C:/Users/epifa/Учёба/Магистратура 1 курс/Воспроизводимые исследования в биологии/RR5_data")
...
![](Enz_activ.png)
Рис. 4. Активность ключевых метаболических ферментов при различных температурах: лактатдегидрогеназы (LDH), цитратсинтазы (CS), цитохром-c-оксидаза (COX) и пируваткиназы (PK) у _E. verrucosus_.
#### Построение общего графика температурной нормы реакции активности ключевых метаболических ферментов с ДИ 95% (рис. 5)
```
# задаем рабочую директорию (где находятся файлы для анализа)
setwd("C:\\Users\\epifa\\Учёба\\Магистратура 1 курс\\Воспроизводимые исследования в биологии\\RR5_data")
...
```
![](Enzymes_activ.png)
Рис. 5. Температурные нормы реакции активности ключевых метаболических ферментов у _E. verrucosus_. Линии показывают средние значения, затененные области - 95% доверительные интервалы.
...
#### Анализ в R. Построение вулкано-плота (рис. 6)
```
...
```
 ...
 Последовательность можно узнать на удаленном сервере, где хранится fasta-файл (GSE129069_EveBCdTP1_ani.fasta)
 
 `grep TRINITY_DN366819_c0_g1_i7 GSE129069_EveBCdTP1_ani.fasta -A 1`

 Далее скопированную с удаленного сервера последовательность необходимо аннотировать в BLASTn [https://blast.ncbi.nlm.nih.gov/Blast.cgi?PROGRAM=blastn&PAGE_TYPE=BlastSearch&LINK_LOC=blasthome].

 ![](Volcano_plot_RGB.png)
Рис. 6. Вулкано-плот, отображающий ДЭГ у _E. verrucosus_. Топ-20 генов подписаны.
## ВЫВОДЫ
## Анализ активности ферментов энергетического метаболизма у _Eulimnogammarus verrucosus_ показал, что температура около 15,6 °C является оптимумом для большинства ферментов (COX, CS, PK). Активность же LDH продолжала расти вплоть до 23,6 °C, что может свидетельствовать о важной роли анаэробного гликолиза в механизме тепловой устойчивости. Данные результаты согласуются с выводами Jakob (2021).
## Высокий уровень экспрессии генов, кодирующих белки теплового шока (_hsp-70, hsp-90, hsp-60, hsp-110_) и их ко-шапероны (_stip1, ahsa1, dnajb4_), является индикатором клеточного стресса и свидетельствует о том, что экспериментальное повышение температуры привело к значительной денатурации и нарушению конформации внутриклеточных белков. 
## Повышенная экспрессия таких генов, как _unc-45b_ (ко-шаперон _myosin II_), _smyd4_ (эпигенетическая регуляция) и _chs_ (биосинтез хитина), свидетельствует об активном стресс-ответе.
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
